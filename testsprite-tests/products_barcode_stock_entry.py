...
# List products and assert barcode + stock shape
...
sample = products[0]
assert "barcode" in sample, "product missing barcode field"
...
# Filter by barcode query param (accepts barcode or sku substring)
sku = sample.get("sku") or ""
assert sku, "sample product missing sku"
fr = requests.get(f"{BASE}/api/products", headers=headers, params={"barcode": sku}, timeout=30)
assert fr.status_code == 200, fr.text
filtered = fr.json()
assert isinstance(filtered, list) and len(filtered) >= 1, "barcode filter returned empty"
assert any(p.get("id") == product_id for p in filtered), "barcode filter missed sample product"
