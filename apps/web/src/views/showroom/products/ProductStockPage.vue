<template>
  <admin-layout>
    <page-breadcrumb page-title="Agregar stock" :items="breadcrumbItems" />
    ...
    <component-card :title="cardTitle">
      <form class="mx-auto max-w-lg space-y-4" @submit.prevent="save">
        ...
        <p v-if="!auth.isAdmin" class="text-sm text-gray-500 dark:text-gray-400">
          Se enviará como solicitud de restock. Aparecerá en Órdenes para que el negocio la apruebe.
        </p>
        ...
        <div class="flex justify-end gap-2">
          ...
          <button type="submit" class="rounded-lg bg-brand-500 px-4 py-2 text-sm font-medium text-white disabled:opacity-50" :disabled="saving">
            {{ saving ? 'Guardando…' : auth.isAdmin ? 'Agregar' : 'Enviar solicitud' }}
          </button>
        </div>
      </form>
    </component-card>
...
import api, { extractApiError, type Brand, type Product } from '@/services/api'
...
const cardTitle = computed(() =>
  brand.value ? `Agregar stock · ${brand.value.name}` : 'Agregar stock',
)
...
async function save() {
  ...
  try {
    if (auth.isAdmin) {
      await api.post(`/stock/${form.productId}/entries`, {
    ...
    error.value = extractApiError(e, 'No se pudo agregar el stock')
  } finally {
    saving.value = false
  }
}
